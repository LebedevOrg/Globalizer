...
// ВАЖНО ПРО КОДИРОВКУ:
// Все строки, которые уходят в Python (doc, имена, сообщения), должны быть
// в UTF-8. MSVC без флага /utf-8 кодирует литералы в cp1251, и Python падает
// при импорте с "initialization failed" (UnicodeDecodeError).
// Поэтому:
//   1) строки docstring в этом файле - только ASCII;
//   2) сообщения из C++ (e.what()) передаются через to_py_str(),
//      который понимает и UTF-8, и cp1251;
//   3) в CMake для цели PYDGlobalizer включён /utf-8.
...
// ============================================================================
// Безопасное преобразование std::string -> Python str.
// Сначала пробуем строгий UTF-8. Если строка пришла в cp1251
// (например, русские сообщения исключений Globalizer), декодируем её
// кодовой страницей Windows. Иначе заменяем плохие байты на "?".
// ============================================================================
...
// ============================================================================
// Валидация задачи без запуска решателя
// ============================================================================
...
// ============================================================================
// Новый интерфейс: solve(problem, params) -> dict
// ============================================================================
...
// ============================================================================
// Старый интерфейс: тонкая обёртка над solve(), возвращает dict
// ============================================================================
...
// ============================================================================
// Биндинг pybind11.
// Имя модуля ОБЯЗАНО совпадать с CMake-целью GLOBALIZER_PYD_NAME и с
// `import PYDGlobalizer` в Python.
// Все docstring ниже - строго ASCII (см. комментарий в начале файла).
// ============================================================================
