...
class MinMaxHeap
{
  ...
  int trickleUp_(unsigned int index) // вспомогательный метод для всплытия
  {
    ...
    {
      TQueueBaseData* i1 = static_cast<TQueueBaseData*>(m_heap[index].pValue);
      TQueueBaseData* i2 = static_cast<TQueueBaseData*>(m_heap[index_grandparent].pValue);
  ...
  int trickleUp(unsigned int index) // размещаем узел на соответствующем уровне (Min или Max)
  {
    ...
    if (isOnMinLevel(index))
    {
      ...
      {
        TQueueBaseData* i1 = static_cast<TQueueBaseData*>(m_heap[index_parent].pValue);
        TQueueBaseData* i2 = static_cast<TQueueBaseData*>(m_heap[index].pValue);
  ...
  void trickleDown_(unsigned int index) // вспомогательный метод для погружения
  {
    ...
    TQueueBaseData* i1 = static_cast<TQueueBaseData*>(m_heap[index].pValue);
    TQueueBaseData* i2 = static_cast<TQueueBaseData*>(m_heap[smallestNode].pValue);
    ...
    if (smallestNode - left > 1)
    {
      ...
      if (m_compare(m_heap[parent(smallestNode)], m_heap[smallestNode]) ^ MaxLevel)
      {
        ...
        i1 = static_cast<TQueueBaseData*>(m_heap[par0].pValue);
        i2 = static_cast<TQueueBaseData*>(m_heap[smallestNode].pValue);
  ...
  void deleteElement(unsigned int index) // удаление элемента из кучи
  {
    ...
    TQueueBaseData* i1 = static_cast<TQueueBaseData*>(m_heap[index].pValue);
    TQueueBaseData* i2 = static_cast<TQueueBaseData*>(m_heap[m_currentheapsize - 1].pValue);
